...
/* ---------- shared <head> ---------- */
// Structured data (schema.org JSON-LD) and link-preview images need absolute URLs, so they're only added when a domain is set.
const ldJson = data => data && origin ? `\n<script type="application/ld+json">${JSON.stringify(Array.isArray(data) && data.length === 1 ? data[0] : data).replace(/</g, "\\u003c")}</script>` : "";
const crumbs = list => ({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: list.map(([name, u], i) => ({ "@type": "ListItem", position: i + 1, name, item: origin + u })) });
const provider = () => ({ "@type": "Organization", name: cfg.siteName, url: origin + "/" });
// Link-preview images from tools/og-images.js (public/og/<name>.png); pages without their own use the site image.
const ogFor = name => ["og/" + name + ".png", "og/site.png"].find(f => name && fs.existsSync(path.join(PUB, f))) || (fs.existsSync(path.join(PUB, "og/site.png")) ? "og/site.png" : "");
function head({ title, desc, prefix, urlPath, scripts, lang = "en", ld = null, og = "site", alt = null }) {
  const img = origin ? ogFor(og) : "";
  const canonical = (origin ? `\n<link rel="canonical" href="${origin}${urlPath}">\n<meta property="og:url" content="${origin}${urlPath}">` : "")
    ...
    + ldJson(ld)
    // English and Spanish versions of the same page point at each other so search engines show the right one.
    + (origin && alt ? Object.entries(alt).map(([l, u]) => `\n<link rel="alternate" hreflang="${l}" href="${origin}${u}">`).join("") + `\n<link rel="alternate" hreflang="x-default" href="${origin}${alt.en}">` : "");
  return `<!doctype html>
<html lang="${lang}">
<head>
<meta charset="utf-8">
...
// The app's own plan builder (assets/core.js) run in a sandbox, so plan summaries match the app exactly.
let PB = null;
function planBuilder() {
  if (PB) return PB;
  const vm = require("vm"), noop = () => {}, el = () => ({ addEventListener: noop, setAttribute: noop, removeAttribute: noop, appendChild: noop, style: {}, classList: { add: noop, remove: noop } });
  const ctx = { console, setTimeout, clearTimeout, navigator: {}, location: { hash: "", search: "", pathname: "/" }, history: {}, matchMedia: () => ({ matches: false, addEventListener: noop }), localStorage: { getItem: () => null, setItem: noop }, addEventListener: noop,
    document: { currentScript: null, addEventListener: noop, documentElement: el(), createElement: el, head: el(), body: el(), querySelector: () => null, querySelectorAll: () => [] } };
  ctx.window = ctx; vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(path.join(PUB, "assets/core.js"), "utf8"), ctx);
...
for (const c of certs) {
  ...
  if (has("handson-es")) plan.hasHandsonEs = true;
  ...
  if (ex && ex.error) { console.error(ex.error); process.exit(1); }
  let qs = ex ? c.questions.concat(ex.questions) : c.questions;
  let W = ex ? { ...ex.whys } : {};
  if (has("whys")) {
    CertHub.addWhys = (id, m) => { if (id === c.id) W = { ...(m || {}), ...W }; };
    delete require.cache[require.resolve(path.join(PUB, "data/whys", c.id + ".js"))];
    require(path.join(PUB, "data/whys", c.id + ".js"));
  }
  const L = ex ? ex.levels : {};
  qs = qs.map(q => { const why = Array.isArray(W[q[0]]) ? W[q[0]] : null, lv = L[q[0]] || null; return why || lv ? [q[0], q[1], q[2], q[3], q[4], q[5], q[6], q[7] ?? null, why, lv] : q; });
  plan.qCount = qs.length;
  if (ex) plan.hasLevels = true;
  delete plan.questions;
  // Every page gets a short summary of each plan (enough for cards, lab links and the daily review);
  // the full weekly plan (data/gen/<id>-plan.js) loads when that certification's page opens.
  const full = planBuilder().buildPlan(plan);
  const HEAVY_DOM = ["topics", "notes", "labs", "study", "labRefs"];
  const lite = { ...plan, lite: true, plan: full.weeks.map(w => [w.title, w.dom, w.labRefs || []]), domains: plan.domains.map(d => Object.fromEntries(Object.entries(d).filter(([k]) => !HEAVY_DOM.includes(k)))) };
  ["weeks", "study", "final", "checkpoints", "phases"].forEach(k => delete lite[k]);
  const detail = Object.fromEntries(["weeks", "study", "final", "checkpoints", "phases", "domains"].filter(k => plan[k] !== undefined).map(k => [k, plan[k]]));
  out(`public/data/gen/${c.id}.js`, `/* Generated by tools/build.js from data/${c.id}.js. Edit that file, not this one. */\nCertHub.register(${JSON.stringify(lite)});\n`);
  out(`public/data/gen/${c.id}-plan.js`, `/* Generated by tools/build.js from data/${c.id}.js. Edit that file, not this one. */\nCertHub.addPlanDetail(${JSON.stringify(c.id)}, ${JSON.stringify(detail)});\n`);
...
// Short entry per lab for cards and counts; the app loads the full lab files when a page needs them.
out("public/data/lab-index.js", `/* Generated by tools/build.js from data/labs-*.js. Edit those files, not this one. */
CertHub.labFiles = ${JSON.stringify(labFiles)};
CertHub.registerLabs(${JSON.stringify(CertHub.labList.map(l => ({ id: l.id, title: l.title, track: l.track, level: l.level, minutes: l.minutes, cost: l.cost, summary: l.summary, stepCount: (l.steps || []).length })))}, { index: true });
`);
...
/* ---------- pages ---------- */
// Header logo and name for the static pages (the app header has the same mark in templates/home.html).
const BRAND_HTML = '<svg class="logo" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><rect width="64" height="64" rx="14" fill="#16202C"/><path d="M13 48 L25 41 L37 32" stroke="#2D5BD0" stroke-width="5" fill="none" stroke-linecap="round" stroke-linejoin="round"/><circle cx="13" cy="48" r="5" fill="#fff" stroke="#2D5BD0" stroke-width="3.5"/><circle cx="25" cy="41" r="5" fill="#fff" stroke="#12806A" stroke-width="3.5"/><circle cx="46" cy="22" r="12" fill="#E8B444"/><path d="M40.5 22.5 l4 4 l7.5 -8" stroke="#16202C" stroke-width="3.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M40 32 l-3 12 l6 -4 l3 6 l3 -12" fill="#E8B444" opacity=".85"/></svg>' + `<span>${esc(cfg.siteName) === "StudyToCert" ? "Study<b>To</b>Cert" : esc(cfg.siteName)}</span>`;
...
out("public/index.html", `${head({ title: cfg.siteName, desc: cfg.description, prefix: "", urlPath: "/", scripts: APP_SCRIPTS, ld: [{ "@context": "https://schema.org", "@type": "WebSite", name: cfg.siteName, url: origin + "/", description: cfg.description }, { "@context": "https://schema.org", ...provider(), description: cfg.description }] })}
<body>
${shell}
</body>
</html>
...
certs.forEach(c => out(`public/${c.id}/index.html`, `${head({
  ...
  prefix: "../", urlPath: `/${c.id}/`, scripts: APP_SCRIPTS, og: c.id,
  ld: [{ "@context": "https://schema.org", "@type": "Course", name: `${c.name} (${c.exam}) study plan`, description: `Free week-by-week ${c.name} ${c.exam} study plan with lessons, labs, quizzes and a practice exam.`, url: `${origin}/${c.id}/`, provider: provider(), isAccessibleForFree: true, inLanguage: "en", educationalLevel: "Beginner to intermediate", teaches: c.domains.map(d => d.name), offers: [{ "@type": "Offer", category: "Free", price: 0, priceCurrency: "USD" }], hasCourseInstance: [{ "@type": "CourseInstance", courseMode: "Online", courseWorkload: `P${c.planWeeks || 8}W` }] },
    crumbs([["Home", "/"], [`${c.short} ${c.exam}`, `/${c.id}/`]])]
})}
...
`);
  items.forEach((x, i) => {
    ...
    out(`public/${c.id}/lessons/${x.slug}/index.html`, `${head({ title: `${l.t.length > 60 ? l.t.slice(0, 58).replace(/[\s,:;]+\S*$/, "") + "…" : l.t} · ${c.short} Lesson`, desc, prefix: "../../../", urlPath: `/${c.id}/lessons/${x.slug}/`, scripts: [], og: c.id, alt: esOf(c.id, l.t) ? { en: `/${c.id}/lessons/${x.slug}/`, es: `/es/${c.id}/lessons/${x.slug}/` } : null,
      ld: [{ "@context": "https://schema.org", "@type": "LearningResource", name: l.t, description: desc, url: `${origin}/${c.id}/lessons/${x.slug}/`, learningResourceType: "Lesson", inLanguage: "en", isAccessibleForFree: true, educationalLevel: "Beginner", ...((CertHub.lessonMeta || {})[c.id] || {}).reviewed ? { dateModified: CertHub.lessonMeta[c.id].reviewed } : {}, teaches: l.terms.map(t => String(t[0]).replace(/`/g, "")).slice(0, 8), provider: provider(), isPartOf: { "@type": "Course", name: `${c.name} (${c.exam}) study plan`, url: `${origin}/${c.id}/` } },
        crumbs([["Home", "/"], [c.short, `/${c.id}/`], ["Lessons", `/${c.id}/lessons/`], [l.t, `/${c.id}/lessons/${x.slug}/`]])] })}
<body>
...
/* ---------- certification facts for the accounts API (class roster summaries) ---------- */
// Domain weights (for the readiness estimate) and the key of every lesson in each plan, the
// same numbers the app uses for "Exam readiness" and "lessons read". No question content.
out("api/src/cert-meta.js", `/* Generated by tools/build.js from the certification and lesson data. Do not edit.
   Used by classes.js to summarize a student's progress for their teacher: each plan's domain
   weights and the keys of its lessons (the same key as lessonKey() in public/assets/engine.js). */
export default {
${certs.map(c => {
  const topics = (c.weeks ? c.weeks.filter(w => w.dom).flatMap(w => w.topics || []) : c.domains.flatMap(d => d.topics || [])).filter(t => !/^Checkpoint test/i.test(t));
  const have = new Set(((CertHub.lessonData || {})[c.id] || []).map(l => l.t));
  return `  ${JSON.stringify(c.id)}: ${JSON.stringify({ domains: c.domains.map(d => [String(d.id), d.w]), lessons: topics.filter(t => have.has(t)).map(lessonKeyOf) })}`;
}).join(",\n")}
};
`);
...
certs.forEach(c => {
  ...
  out(`public/es/${c.id}/lessons/index.html`, `${head({ title: `Lecciones de ${c.short} ${c.exam}`, desc: `${items.length} lecciones gratuitas en español para ${c.name} (${c.exam}).`, prefix: "../../../", urlPath: `/es/${c.id}/lessons/`, scripts: [], lang: "es", alt: { en: `/${c.id}/lessons/`, es: `/es/${c.id}/lessons/` } })}
<body>
<header class="top"><div class="bar"><a class="brand" href="../../../">${BRAND_HTML}</a></div></header>
<main class="wrap lesson-page">
...
`);
  items.forEach((x, i) => {
    ...
    out(`public/es/${c.id}/lessons/${x.slug}/index.html`, `${head({ title: `${l.tt.length > 60 ? l.tt.slice(0, 58) + "…" : l.tt} · Lección de ${c.short}`, desc: String(l.body[0] || "").replace(/`/g, "").slice(0, 155), prefix: "../../../../", urlPath: `/es/${c.id}/lessons/${x.slug}/`, scripts: [], lang: "es", alt: { en: `/${c.id}/lessons/${x.slug}/`, es: `/es/${c.id}/lessons/${x.slug}/` } })}
<body>
<header class="top"><div class="bar"><a class="brand" href="../../../../">${BRAND_HTML}</a></div></header>
<main class="wrap lesson-page">
...
<div class="panel startcard"><div class="grow"><strong>Estudia ${esc(c.short)} gratis</strong><br><span class="note">Plan semana a semana con lecciones, cuestionarios, simulaciones de examen y práctica, en español.</span></div><a class="btn sm" href="../../../../${c.id}/?lang=es">Abrir el plan de estudio</a></div>
...
/* ---------- Spanish hub (/es/) and Spanish cheat sheets (/es/<cert>/cheat-sheet/) ---------- */
{
  ...
  withEs.forEach(c => {
    const ls = esList(c); if (!ls.length) return;
    const plan = (c.weeks ? c.weeks.filter(w => w.dom).map(w => [w.dom, w.topics]) : c.domains.map(d => [d.id, d.topics || []]));
    const byDom = d => ls.filter(l => (plan.find(([, ts]) => ts.includes(l.t)) || [])[0] === d.id);
    out(`public/es/${c.id}/cheat-sheet/index.html`, `${head({ title: `Hoja de repaso de ${c.short} ${c.exam}`, desc: `Hoja de repaso gratuita de ${c.name} (${c.exam}): cada consejo para el examen y término clave por dominio, lista para imprimir.`, prefix: "../../../", urlPath: `/es/${c.id}/cheat-sheet/`, scripts: [], lang: "es", alt: { en: `/${c.id}/cheat-sheet/`, es: `/es/${c.id}/cheat-sheet/` } })}
<body>
<header class="top"><div class="bar"><a class="brand" href="../../../">${BRAND_HTML}</a></div></header>
<main class="wrap lesson-page">
<p class="crumbs"><a href="../../">Lecciones en español</a> / <a href="../lessons/">${esc(c.short)}</a> / Hoja de repaso · <a href="../../../${c.id}/cheat-sheet/" hreflang="en" lang="en">English</a></p>
<h1>Hoja de repaso de ${esc(c.short)} ${esc(c.exam)}</h1>
<p class="meta">Cada consejo para el examen y término clave de las lecciones gratuitas de ${esc(c.short)}, por dominio. Usa la opción Imprimir de tu navegador para guardarla como PDF.</p>
${c.domains.map(d => { const dl = byDom(d); if (!dl.length) return ""; const seen = new Set(); return `<h2>Dominio ${d.id}: ${esc(d.name)} (${d.w}%)</h2><div class="panel cheat"><h3>Consejos para el examen</h3><ul class="clean">${dl.map(l => `<li>${inl(l.tip)}</li>`).join("")}</ul><h3>Términos clave</h3><dl class="terms">${dl.flatMap(l => l.terms || []).filter(([t]) => { const k = t.toLowerCase(); if (seen.has(k)) return false; seen.add(k); return true; }).map(([a, b]) => `<dt>${inl(a)}</dt><dd>${inl(b)}</dd>`).join("")}</dl></div>`; }).join("\n")}
<div class="panel startcard"><div class="grow"><strong>Estudia ${esc(c.short)} gratis</strong><br><span class="note">Plan semana a semana con lecciones, cuestionarios, simulaciones de examen y práctica, en español.</span></div><a class="btn sm" href="../../../${c.id}/?lang=es">Abrir el plan de estudio</a></div>
</main>
</body>
</html>
`);
    lessonPages.push([`/es/${c.id}/cheat-sheet/`, c.lastVerified]);
  });
  if (withEs.length) {
    out("public/es/index.html", `${head({ title: `${cfg.siteName} en español: planes de estudio gratuitos para certificaciones de TI`, desc: `Lecciones, hojas de repaso y planes de estudio gratuitos en español para ${withEs.length} certificaciones de TI, nube y ciberseguridad.`, prefix: "../", urlPath: "/es/", scripts: [], lang: "es", alt: { en: "/", es: "/es/" } })}
<body>
<header class="top"><div class="bar"><a class="brand" href="../">${BRAND_HTML}</a></div></header>
<main class="wrap lesson-page">
<p class="crumbs"><a href="../" hreflang="en" lang="en">English</a></p>
<h1>Estudia para certificarte, en español</h1>
<p class="meta">Planes de estudio gratuitos semana a semana para certificaciones de TI, nube y ciberseguridad: lecciones, cuestionarios, simulaciones de examen, práctica y repaso espaciado. Sin registro y sin anuncios.</p>
<div class="btns"><a class="btn" href="../?lang=es">Abrir StudyToCert en español</a></div>
${(CertHub.tracks || []).map(t => { const cs = t.certs.map(id => withEs.find(c => c.id === id)).filter(Boolean); return cs.length ? `<h2>${esc(TRACK_ES[t.id] || t.name)}</h2><div class="panel"><ul class="clean">${cs.map(c => `<li><strong>${esc(c.short)}</strong> ${esc(c.exam)}: <a href="${c.id}/lessons/">${esList(c).length} lecciones</a> · <a href="${c.id}/cheat-sheet/">hoja de repaso</a> · <a href="../${c.id}/?lang=es">plan de estudio</a></li>`).join("")}</ul></div>` : ""; }).join("\n")}
</main>
</body>
</html>
`);
...
`);
    ...
    careers.forEach(c => {
      out(`public/careers/${c.track}/index.html`, `${head({ title: `${c.title} · ${cfg.siteName}`, desc: String(c.intro).split("\n")[0].slice(0, 155), prefix: "../../", urlPath: `/careers/${c.track}/`, scripts: [], og: "careers", ld: crumbs([["Home", "/"], ["Careers", "/careers/"], [c.title, `/careers/${c.track}/`]]) })}
<body>
${top("../../")}
<main class="wrap lesson-page">
...
// Android app (Trusted Web Activity) link: set "android": { "package", "sha256": [signing-key fingerprints] } in site.config.json.
// See docs/PLAY_STORE.md. Without it, no assetlinks.json is published.
{
  const a = cfg.android || {}, pkg = String(a.package || "").trim(), fps = [].concat(a.sha256 || []).map(x => String(x).trim().toUpperCase()).filter(Boolean);
  if (pkg || fps.length) {
    if (!/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$/i.test(pkg)) { console.error(`site.config.json android.package must look like com.example.app, got "${pkg}"`); process.exit(1); }
    if (!fps.length || !fps.every(f => /^([0-9A-F]{2}:){31}[0-9A-F]{2}$/.test(f))) { console.error("site.config.json android.sha256 must list SHA-256 fingerprints like AB:CD:...(32 pairs)"); process.exit(1); }
    out("public/.well-known/assetlinks.json", JSON.stringify([{ relation: ["delegate_permission/common.handle_all_urls"], target: { namespace: "android_app", package_name: pkg, sha256_cert_fingerprints: fps } }], null, 2) + "\n");
  } else if (!CHECK && fs.existsSync(path.join(PUB, ".well-known/assetlinks.json"))) fs.rmSync(path.join(PUB, ".well-known/assetlinks.json"));
}
