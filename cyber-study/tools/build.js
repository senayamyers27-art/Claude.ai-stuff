...
for (const c of certs) {
  ...
  // Pages only ask for lessons that exist (data/lessons/<id>.js).
  if (fs.existsSync(path.join(PUB, "data/lessons", c.id + ".js"))) plan.hasLessons = true;
...
walk(PUB).filter(f => !/(^|\/)(sw\.js|_headers|_redirects|robots\.txt|sitemap\.xml|OFL\.txt)$/.test(f) && !f.includes(".well-known"))
  .sort().forEach(f => {
    ...
    const lesson = rel.startsWith("/data/lessons/");
