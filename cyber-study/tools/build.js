...
function head({ title, desc, prefix, urlPath, scripts, lang = "en", ld = null, og = "site" }) {
...
`);
  items.forEach((x, i) => {
    ...
    out(`public/${c.id}/lessons/${x.slug}/index.html`, `${head({ title: `${l.t.length > 60 ? l.t.slice(0, 58).replace(/[\s,:;]+\S*$/, "") + "…" : l.t} · ${c.short} Lesson`, desc, prefix: "../../../", urlPath: `/${c.id}/lessons/${x.slug}/`, scripts: [], og: c.id,
...
certs.forEach(c => {
  ...
  out(`public/${c.id}/cheat-sheet/index.html`, `${head({ title: `${c.short} ${c.exam} Cheat Sheet`, desc: `Free ${c.name} (${c.exam}) cheat sheet: every exam tip and key term by domain, ready to print.`, prefix: "../../", urlPath: `/${c.id}/cheat-sheet/`, scripts: [], og: c.id, ld: crumbs([["Home", "/"], [c.short, `/${c.id}/`], ["Cheat sheet", `/${c.id}/cheat-sheet/`]]) })}
<body>
<header class="top"><div class="bar"><a class="brand" href="../../">${esc(cfg.siteName)}</a></div></header>
<main class="wrap lesson-page">
