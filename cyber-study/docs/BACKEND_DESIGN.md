...
cohorts, invites, pilot seats, instructor summary and CSV, class mode (free teacher classes with
join codes and consented progress summaries), audit log and rate limits.
...
-- class mode (free, no org or seats): any user can teach up to 20 classes
classes(id TEXT PK, teacher_id FK, teacher_name, name, cert_id NULL, join_code TEXT UNIQUE, created_at, updated_at)
class_members(id TEXT PK, class_id FK, user_id FK, display_name, show_email INT, consented_at, joined_at,
...
| `GET /v1/classes` · `POST /v1/classes` | user | classes I teach and classes I'm in; create a class (max 20) |
| `PUT /v1/classes/:id` · `DELETE /v1/classes/:id` | teacher | rename or change the certification; delete the class |
| `POST /v1/classes/:id/code` | teacher | rotate the join code (the old one stops working) |
| `GET /v1/classes/join/:code` | user | preview: class name, teacher's display name, what gets shared |
| `POST /v1/classes/join/:code` | user | join with `{ consent: true, displayName, showEmail }` |
| `DELETE /v1/classes/:id/membership` | student | leave the class (sharing stops at once) |
| `DELETE /v1/classes/:id/students/:memberId` | teacher | remove a student |
| `GET /v1/classes/:id/roster` · `GET /v1/classes/:id/roster.csv` | teacher | per-student progress summary, CSV export |
| `GET /v1/account/export` · `DELETE /v1/account` | user | download everything; delete account |

## Class mode

Free and separate from organizations: a teacher (any signed-in user) creates a class and shares
its join link (`#join-<code>`). No seats, no billing and no Pro gate.

- **Join codes**: 10 characters from a 32-letter alphabet without look-alikes (50 bits from
  `crypto.getRandomValues`); lookups are rate limited per user and per IP. The teacher can rotate
  the code; students already in the class stay.
- **Consent**: the join page shows the class, the teacher's display name and exactly what will
  be shared. The server refuses to join without `consent: true`, and records `consented_at`.
- **What teachers see**: the student's chosen display name (email only if the student opts in)
  and, per certification studied: readiness (the app's estimate, computed with the plan's domain
  weights), lessons read out of the plan's total, best practice exam, questions answered,
  hands-on exercises and simulations done, last active; plus labs finished. Computed server-side
  (`api/src/classes.js`) from the synced progress documents; answers, review queues, quiz
  history and lab notes never leave the server. Domain weights and lesson keys come from
  `api/src/cert-meta.js`, generated by `tools/build.js`.
- **Leaving**: a student can leave at any time; the membership row is deleted, so the next
  roster no longer includes them. Deleting either account removes its classes or memberships.
- **Isolation**: a class someone else teaches, or doesn't exist, answers `404`; students and
  non-members can't read the roster; members are addressed by a `mem_` id, never a user id.
