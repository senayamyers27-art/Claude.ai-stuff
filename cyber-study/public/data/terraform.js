...
CertHub.register({
  ...
  domains: [
    ...
    },
    {
      ...
      labs: [
        "Create a free HCP Terraform account and organization, run `terraform login`, add a `cloud` block to a `random`/`local`-free config (only `random` and `terraform_data`) and run a CLI-driven plan remotely.",
  ...
  questions: [
    ...
    ["tf1",0,1,"A team rebuilds a test environment by hand each sprint and the builds never match. Which IaC benefit most directly fixes this?",["Lower cloud list prices for the same resources","Repeatable builds from the same versioned code","Faster network throughput between the servers","Automatic replacement of the operating system"],1,"Code in version control produces the same environment every time. IaC doesn't change prices, bandwidth or OS choice.","Objective 1b"],
    ["tf2",0,1,"Which statement best describes Terraform's declarative approach?",["You write each API call in the order it must run","You describe the end state and Terraform works out the steps","You record console clicks and Terraform replays them","You write shell scripts that Terraform schedules"],1,"Terraform configuration declares the desired result; Terraform builds the dependency graph and decides the actions. Listing each call in order is the imperative style.","Objective 1a"],
    ["tf3",0,1,"A company runs workloads on AWS and Azure and manages DNS with Cloudflare. How can Terraform help?",["It converts AWS resources into Azure resources on demand","It needs a separate Terraform edition for each cloud","One workflow and language manage all three through providers","It can only manage one provider per state file"],2,"Providers let a single configuration and plan/apply workflow cover many platforms. Terraform doesn't translate resources between clouds, and one state can hold several providers.","Objective 1c"],
    ...
    ["tf5",0,1,"Which task is Terraform designed for, compared with a configuration management tool?",["Editing application config files inside running VMs","Patching packages on long-lived servers each night","Provisioning and managing the lifecycle of infrastructure","Collecting logs from servers and sending alerts"],2,"Terraform provisions and manages infrastructure resources through APIs. Package patching and in-guest configuration are what tools like Ansible or Chef focus on.","Objective 1a"],
    ["tf6",0,1,"A security lead wants every infrastructure change reviewed before it happens. Which IaC practice supports this?",["Giving every engineer console admin rights","Storing configuration in Git and reviewing plans in pull requests","Running terraform apply -auto-approve on laptops","Making changes first and documenting them later"],1,"Version-controlled code lets changes and their plans be reviewed and approved. Broad console access and auto-approve bypass review.","Objective 1b"],
    ["tf7",0,1,"What lets Terraform manage services such as GitHub teams or Datadog monitors, not just cloud servers?",["Terraform's built-in SaaS connector","A provider plugin for each service's API","A custom Terraform edition for SaaS","Manual steps recorded in the state file"],1,"Any service with an API can have a provider, which is why Terraform is service-agnostic. There is no special SaaS edition.","Objective 1c"],
    ...
    ["tf9",0,2,"A developer runs terraform init and gets a newer provider patch than a teammate did last week. What stops this drift between machines?",["Committing the .terraform.lock.hcl file","Committing the .terraform directory","Deleting the provider cache every run","Setting TF_LOG to TRACE"],0,"The dependency lock file records the selected provider versions and checksums; committed to VCS, everyone gets the same builds. The .terraform directory holds downloaded plugins and should not be committed.","Objective 2a"],
    ["tf10",0,2,"The lock file pins aws 5.40.0, and the constraint ~> 5.40 would allow 5.72. How do you move to the newest allowed version?",["Run terraform validate","Run terraform init -upgrade","Run terraform apply -refresh-only","Delete terraform.tfstate"],1,"`init -upgrade` ignores the locked versions, picks the newest within the constraints and updates the lock file. Validate and refresh-only don't touch providers.","Objective 2a"],
    ["tf11",0,2,"What is a Terraform provider?",["A plugin that lets Terraform call a platform's API","A file that stores the current resource attributes","A reusable folder of resources with inputs","The company hosting your remote state"],0,"Providers are plugins that implement resource types and data sources by calling APIs. State stores attributes; modules are reusable folders.","Objective 2b"],
    ["tf12",0,2,"A configuration has provider \"aws\" { region = \"us-east-1\" } and provider \"aws\" { alias = \"west\" region = \"us-west-2\" }. How does a resource use the second one?",["region = aws.west","provider = aws.west","provider = \"west\"","alias = aws.west"],1,"The `provider` meta-argument takes an unquoted reference `<name>.<alias>`. Resources don't take an `alias` argument.","Objective 2c"],
    ["tf13",0,2,"source = \"hashicorp/random\" is shorthand for which full provider address?",["github.com/hashicorp/random","registry.terraform.io/hashicorp/random","releases.hashicorp.com/random","app.terraform.io/hashicorp/random"],1,"When no hostname is given, Terraform uses the public registry at registry.terraform.io.","Objective 2a"],
    ["tf14",0,2,"Which is NOT a purpose of Terraform state?",["Mapping resources in code to real object IDs","Tracking metadata such as dependencies","Caching attributes to speed up planning","Storing the provider plugin binaries"],3,"Provider binaries live in the .terraform directory or a plugin cache. State maps real objects, tracks metadata and caches attributes.","Objective 2d"],
    ["tf15",0,2,"A database password is set through a sensitive variable. Where can the value still be read in plain text?",["In the terraform plan console output","In the terraform.tfstate file","In the output of terraform fmt","Nowhere; sensitive values are encrypted"],1,"`sensitive` only redacts CLI display. The value is still stored in state, so state must be protected.","Objective 2d"],
    ["tf16",0,2,"A terraform block contains required_version = \">= 1.12.0\". An engineer runs Terraform 1.9. What happens?",["Terraform upgrades itself automatically","Terraform returns an error before doing any work","The setting only affects providers","Terraform continues but prints a warning"],1,"`required_version` constrains the Terraform CLI version; a version outside the range makes Terraform stop with an error.","Objective 2a"],
    ["tf17",0,2,"Which registry tier describes providers owned and maintained by HashiCorp?",["Official","Partner","Community","Archived"],0,"Official providers are owned and maintained by HashiCorp. Partner providers are maintained by technology partners, and community providers by individuals or groups.","Objective 2b"],

    /* Domain 3: Core workflow (17) */
    ["tf18",0,3,"An engineer clones a repository with Terraform code and wants to run a plan. What must run first?",["terraform init","terraform apply","terraform refresh","terraform fmt"],0,"init sets up the backend and downloads providers and modules; plan fails without it.","Objective 3b"],
    ["tf19",0,3,"Which step in the core workflow lets a reviewer see exactly what will be created, changed or destroyed?",["Write","Plan","Init","Validate"],1,"`terraform plan` compares configuration with state and real infrastructure and shows proposed actions.","Objective 3a"],
    ["tf20",0,3,"A pipeline must fail if any .tf file is not in canonical format, without rewriting files. Which command fits?",["terraform validate -json","terraform fmt -check -recursive","terraform plan -lock=false","terraform init -reconfigure"],1,"`fmt -check` exits non-zero when files need formatting and changes nothing; `-recursive` includes subdirectories. validate checks correctness, not style.","Objective 3g"],
    ["tf21",0,3,"What does terraform validate check?",["That the provider credentials are valid","That the syntax and references are internally consistent","That resources exist in the cloud account","That the state file matches reality"],1,"validate checks syntax, types and references in the configuration without calling provider APIs or reading remote state.","Objective 3c"],
    ["tf22",0,3,"A team wants CI to apply exactly the plan a reviewer approved. Which pair of commands does this?",["terraform plan, then terraform apply -auto-approve","terraform plan -out=tfplan, then terraform apply tfplan","terraform validate, then terraform apply","terraform refresh, then terraform apply"],1,"A saved plan file freezes the reviewed actions and apply runs it without a prompt. A fresh apply could produce a different plan.","Objective 3d"],
    ["tf23",0,3,"In a plan, a resource is shown with the symbol -/+. What does it mean?",["It will be updated in place","It will be destroyed and then recreated","It will be read as a data source","It will be imported into state"],1,"-/+ means replacement: destroy then create. ~ is an in-place update.","Objective 3d"],
    ["tf24",0,3,"A container is healthy but you need Terraform to recreate it. What is the current recommended command?",["terraform taint docker_container.web","terraform apply -replace=docker_container.web","terraform state rm docker_container.web","terraform destroy -auto-approve"],1,"`-replace` plans a replacement for that one resource and shows it in the plan. taint is deprecated; state rm would forget the resource instead.","Objective 3e"],
    ["tf25",0,3,"How can you preview what terraform destroy would remove without deleting anything?",["terraform plan -destroy","terraform destroy -check","terraform validate -destroy","terraform show -destroy"],0,"`plan -destroy` creates a speculative destroy plan. destroy has no -check option.","Objective 3f"],
    ["tf26",0,3,"After adding a new module block to the configuration, terraform plan reports \"Module not installed\". What fixes it?",["terraform fmt","terraform init","terraform validate","terraform output"],1,"init (or terraform get) downloads new modules. Any new provider or module requires init.","Objective 3b"],
    ["tf27",0,3,"You moved the backend to a new bucket but don't want to copy the old state, just start using the new settings. Which command applies?",["terraform init -reconfigure","terraform init -upgrade","terraform apply -refresh-only","terraform validate -backend"],0,"`-reconfigure` ignores the existing backend setup and configures the new one without migrating state. `-migrate-state` would copy it.","Objective 3b"],
    ["tf28",0,3,"Running terraform apply without a saved plan does what before making changes?",["Applies immediately with no output","Creates a plan and asks you to approve it","Runs terraform fmt on all files","Destroys all resources first"],1,"Without a plan file, apply generates a plan and waits for you to type yes (unless -auto-approve is used).","Objective 3e"],
    ["tf29",0,3,"Which plan symbol shows an attribute being updated without replacing the resource?",["+","-","~","<="],2,"~ means update in place. + is create, - is destroy, and <= marks a data source read.","Objective 3d"],
    ["tf30",0,3,"A teammate wants to pass a different variable file for staging at plan time. Which syntax is correct?",["terraform plan -var-file=staging.tfvars","terraform plan -tfvars staging","terraform plan --vars=staging.tfvars","terraform plan -input=staging.tfvars"],0,"`-var-file` loads variable values from the named file. The other flags don't exist or mean something else (-input controls prompting).","Objective 3d"],
    ["tf31",0,3,"Why is terraform destroy considered part of the core workflow rather than just a cleanup step?",["It rewrites the configuration files","It removes all resources Terraform manages in that state","It deletes only resources that drifted","It removes providers from the lock file"],1,"destroy removes everything tracked in the current state, which is useful for short-lived environments. It doesn't touch code or the lock file.","Objective 3f"],
    ["tf32",0,3,"An engineer runs terraform fmt in a directory. What does it change?",["Resource names to match a naming standard","Indentation and alignment to the canonical style","Deprecated arguments to their replacements","Provider versions to the latest release"],1,"fmt only rewrites layout (spacing, alignment, indentation). It doesn't rename resources or change logic.","Objective 3g"],
    ["tf33",0,3,"You want terraform plan to skip checking real infrastructure to speed up a quick review. Which option does that?",["-refresh=false","-refresh-only","-lock=false","-parallelism=1"],0,"-refresh=false uses cached state without querying providers. -refresh-only does the opposite: it plans only state updates from reality.","Objective 3d"],
    ["tf34",0,3,"During an outage you need to apply a fix to one resource only. Which option limits the run, and how should it be treated?",["-target, used only for exceptional cases","-replace, used for all normal changes","-refresh-only, used after every apply","-lock=false, used in CI by default"],0,"-target focuses plan/apply on given addresses and their dependencies; HashiCorp recommends it only for exceptional situations, not routine use.","Objective 3d"],

    /* Domain 4: Configuration (20) */
    ["tf35",0,4,"Given: data \"docker_image\" \"nginx\" { name = \"nginx:latest\" }. How do you reference its ID in a resource?",["docker_image.nginx.id","data.docker_image.nginx.id","data.nginx.id","var.docker_image.nginx.id"],1,"Data sources are referenced as data.TYPE.NAME.ATTRIBUTE. Without the data prefix, the address points to a managed resource.","Objective 4a"],
    ["tf36",0,4,"What is the main difference between a resource block and a data block?",["Data blocks create objects; resources only read them","Resources are managed by Terraform; data blocks only read existing information","Data blocks are only allowed inside modules","Resources cannot reference data blocks"],1,"Resource blocks create, update and delete objects. Data blocks read information from providers without managing it.","Objective 4a"],
    ["tf37",0,4,"Given: resource \"local_file\" \"cfg\" { count = 3  filename = \"cfg-${count.index}.txt\"  content = \"x\" }. Which files are created?",["cfg-1.txt, cfg-2.txt, cfg-3.txt","cfg-0.txt, cfg-1.txt, cfg-2.txt","cfg-count.txt three times","cfg-3.txt only"],1,"count.index starts at 0, so three instances get indexes 0, 1 and 2.","Objective 4e"],
    ["tf38",0,4,"Given: for_each = toset([\"dev\", \"prod\"]) on a resource. What are each.key and each.value for the first instance?",["0 and \"dev\"","\"dev\" and \"dev\"","\"dev\" and null","\"key\" and \"dev\""],1,"For a set of strings, each.key and each.value are both the element.","Objective 4e"],
    ["tf39",0,4,"A list of five servers is managed with count. Removing the second item recreates or changes several later servers. What change avoids this?",["Use for_each over a map or set keyed by name","Add depends_on to every server","Set create_before_destroy on the servers","Use -target to apply one server at a time"],0,"count addresses instances by index, so removing one shifts the rest. for_each uses stable keys, so only the removed key is affected.","Objective 4e"],
    ["tf40",0,4,"Variable region has default \"us-east-1\". TF_VAR_region=\"eu-west-1\" is set, terraform.tfvars sets \"us-west-2\", and the command includes -var=\"region=ap-south-1\". Which value is used?",["us-east-1","eu-west-1","us-west-2","ap-south-1"],3,"Command-line -var and -var-file have the highest precedence, then *.auto.tfvars, terraform.tfvars, and environment variables; the default is used only if nothing else sets it.","Objective 4c"],
    ["tf41",0,4,"Given: variable \"ports\" { type = list(number) }. Which value is valid?",["[80, 443]","{ http = 80 }","\"80,443\"","[\"http\", \"https\"]"],0,"list(number) needs an ordered list of numbers. A map, a single string or a list of words cannot convert.","Objective 4d"],
    ["tf42",0,4,"Which type fits a value like { name = \"web\", port = 8080, public = true } with fixed attribute names of different types?",["map(string)","list(any)","object({ name = string, port = number, public = bool })","set(string)"],2,"An object type has named attributes each with its own type. A map requires all values to be the same type.","Objective 4d"],
    ["tf43",0,4,"What does [for s in var.names : upper(s)] return when var.names is [\"a\", \"b\"]?",["{ a = \"A\", b = \"B\" }","[\"A\", \"B\"]","\"AB\"","[\"a\", \"b\", \"A\", \"B\"]"],1,"A for expression in square brackets returns a list with the transformed elements. Curly braces with => would produce a map.","Objective 4e"],
    ["tf44",0,4,"Resource aws_instance.web uses count = 3. How do you output a list of all three IDs?",["aws_instance.web.id","aws_instance.web[*].id","aws_instance.web[3].id","aws_instance[*].web.id"],1,"The splat expression [*] collects an attribute from every instance into a list. Without an index, a counted resource can't be read as one object.","Objective 4e"],
    ["tf45",0,4,"A container resource references docker_image.app.image_id in its image argument. What does Terraform infer?",["Nothing; you must add depends_on","An implicit dependency so the image is handled first","That both resources must be in one module","That the image must be a data source"],1,"References create implicit dependencies; Terraform orders the graph from them. depends_on is only needed for hidden dependencies.","Objective 4f"],
    ["tf46",0,4,"An application server needs a policy to be attached first, but no argument on the server references the policy. What should you add?",["depends_on = [aws_iam_role_policy.app]","lifecycle { prevent_destroy = true }","count = 1 on the policy","A data block for the policy"],0,"depends_on declares a dependency Terraform can't see from references. prevent_destroy and count don't affect ordering.","Objective 4f"],
    ["tf47",0,4,"Replacing a load balancer certificate causes downtime because the old one is deleted first. Which setting fixes the order?",["lifecycle { ignore_changes = all }","lifecycle { create_before_destroy = true }","depends_on = [self]","lifecycle { prevent_destroy = true }"],1,"create_before_destroy makes Terraform create the replacement before destroying the old object. prevent_destroy would block the replacement entirely.","Objective 4f"],
    ["tf48",0,4,"An autoscaler changes a group's desired_capacity outside Terraform, and every plan tries to reset it. What is the cleanest fix?",["lifecycle { ignore_changes = [desired_capacity] }","Run terraform apply -refresh=false every time","Remove the resource from state","Add depends_on to the autoscaler"],0,"ignore_changes tells Terraform not to plan updates for those attributes after creation. The other options hide or break management.","Objective 4f"],
    ["tf49",0,4,"You want terraform plan to fail if var.env is not dev, test or prod. Which block does this?",["A precondition on the provider","A validation block inside the variable","A check block with an assert","An output with sensitive = true"],1,"Variable validation blocks reject bad input with your error message. A check block would only warn.","Objective 4g"],
    ...
    ["tf51",0,4,"Where do you place a postcondition that verifies an attribute of a resource after it is created?",["Inside the terraform block","Inside the resource's lifecycle block","Inside the provider block","Inside a locals block"],1,"precondition and postcondition blocks go inside a resource's (or data source's) lifecycle block, or in an output for preconditions.","Objective 4g"],
    ["tf52",0,4,"A module passes a database password to a provider and the team must keep it out of both plan files and state. Which approach fits Terraform 1.12?",["Mark the variable sensitive = true only","Use an ephemeral variable passed to a write-only argument","Store the password in a local value","Put the password in the output block"],1,"Ephemeral values are never persisted, and write-only arguments accept them without storing them. sensitive only hides display; the value still lands in state.","Objective 4h"],
    ["tf53",0,4,"What is the recommended way to supply short-lived cloud credentials to Terraform instead of hard-coding them?",["Put the keys in terraform.tfvars in Git","Generate them from Vault or use environment variables","Add them to a README in the repository","Store them as default values in variables.tf"],1,"Secrets should come from a secrets manager such as Vault (for example dynamic credentials) or the environment, never from committed files.","Objective 4h"],
    ["tf54",0,4,"Which command gives an interactive prompt for testing expressions such as cidrsubnet(\"10.0.0.0/16\", 8, 1)?",["terraform console","terraform show","terraform graph","terraform output"],0,"terraform console evaluates expressions and functions against the current configuration and state. The result here is 10.0.1.0/24.","Objective 4e"],

    /* Domain 5: Modules (10) */
    ["tf55",0,5,"Which module source refers to a local directory?",["source = \"modules/network\"","source = \"./modules/network\"","source = \"local::modules/network\"","source = \"file:modules/network\""],1,"Local paths must begin with ./ or ../; without them Terraform treats the string as a registry address.","Objective 5a"],
    ["tf56",0,5,"A module block uses source = \"hashicorp/consul/aws\". Where does Terraform get it?",["From a GitHub repository with that name","From the public Terraform Registry","From a local folder named hashicorp","From the HCP Terraform state"],1,"The NAMESPACE/NAME/PROVIDER format is a registry address on registry.terraform.io.","Objective 5a"],
    ["tf57",0,5,"For which module source can you use the version argument?",["A local path","A module from a registry","A plain HTTP URL","A Git repository"],1,"The version argument works only for registry sources (public or private). Git sources pin with ?ref= in the URL.","Objective 5d"],
    ["tf58",0,5,"A child module declares variable \"name\". How does the root module give it a value?",["Set name as an argument in the module block","Set TF_VAR_module_name","Declare the same variable in the root module only","Reference module.child.var.name"],0,"Inputs are passed as arguments in the module block. A root variable with the same name isn't passed automatically.","Objective 5b"],
    ["tf59",0,5,"The root module needs the container ID created inside module \"web\". What is required?",["Nothing; use module.web.docker_container.this.id","An output in the child module, read as module.web.OUTPUT_NAME","A data source in the root module","A depends_on in the child module"],1,"Only outputs cross the module boundary. Internal resources of a child can't be referenced directly.","Objective 5b"],
    ["tf60",0,5,"A module should use an aliased provider configuration aws.west. How is it passed?",["providers = { aws = aws.west } in the module block","provider = \"west\" inside the child's resources","region = \"us-west-2\" in the module block","Aliased providers are inherited automatically"],0,"The providers map passes specific provider configurations to a child. Only default (unaliased) configurations are inherited automatically.","Objective 5c"],
    ["tf61",0,5,"A Git-sourced module must stay on tag v1.4.0. Which source is correct?",["git::https://example.com/net.git?ref=v1.4.0","git::https://example.com/net.git with version = \"1.4.0\"","https://example.com/net.git#v1.4.0","git::https://example.com/net.git//v1.4.0"],0,"Git sources select a tag, branch or commit with ?ref=. The version argument doesn't apply to Git, and // selects a subdirectory.","Objective 5d"],
    ...
    ["tf63",0,5,"Given: module \"bucket\" { source = \"./modules/bucket\"  for_each = toset([\"logs\", \"data\"])  name = each.key }. How is the logs instance addressed?",["module.bucket.logs","module.bucket[\"logs\"]","module.logs.bucket","module.bucket[0]"],1,"Instances created with for_each are addressed by key in square brackets.","Objective 5c"],
    ...
    ["tf65",0,6,"No backend is configured. Where does Terraform keep state for the default workspace?",["In terraform.tfstate in the working directory","In .terraform.lock.hcl","In HCP Terraform automatically","In the provider's cloud account"],0,"The local backend is the default and writes terraform.tfstate in the working directory.","Objective 6a"],
    ["tf66",0,6,"Two engineers run terraform apply at the same time against the same remote state. What prevents corruption?",["State locking by the backend","The dependency lock file","terraform fmt","Provider aliases"],0,"Backends that support locking block a second write until the first finishes. The dependency lock file is about provider versions.","Objective 6b"],
    ["tf67",0,6,"A CI job crashed and left the state locked. Nothing else is running. What should you do?",["Delete the state file","Run terraform force-unlock with the lock ID","Run terraform init -upgrade","Run terraform apply -lock=true"],1,"force-unlock removes a stale lock. Only use it after confirming no run is active; deleting state loses tracking of every resource.","Objective 6b"],
    ["tf68",0,6,"Given: terraform { backend \"s3\" { bucket = var.state_bucket } }. What happens at init?",["It works if the variable has a default","It fails because backend blocks can't use variables","Terraform prompts for the bucket later during apply","Terraform silently uses local state"],1,"Backend configuration is read before variables exist, so references aren't allowed. Use -backend-config for dynamic values.","Objective 6c"],
    ["tf69",0,6,"Each environment uses a different state bucket, but the team wants one backend block. What does Terraform support?",["Partial configuration with -backend-config at init","Using terraform.workspace in the bucket name","Setting the bucket with TF_VAR_bucket","A for_each on the backend block"],0,"Partial backend configuration leaves out settings and supplies them from a file or key=value with -backend-config.","Objective 6c"],
    ["tf70",0,6,"You added a remote backend to a project that already has local state. Which command moves the existing state?",["terraform init -migrate-state","terraform state push","terraform apply -refresh-only","terraform workspace new remote"],0,"init -migrate-state copies existing state into the newly configured backend after confirmation.","Objective 6c"],
    ["tf71",0,6,"Someone changed a container's settings with the Docker CLI. How do you see the drift without proposing changes to infrastructure?",["terraform plan -refresh-only","terraform validate","terraform fmt -diff","terraform state rm"],0,"A refresh-only plan shows how state would change to match reality without planning infrastructure updates.","Objective 6d"],
    ["tf72",0,6,"A refresh-only plan shows a tag was edited manually, and the team decides the manual value is correct. What should they do?",["Apply the refresh-only plan and update the code to match","Run terraform destroy and apply again","Delete the resource from state","Nothing; drift fixes itself on the next plan"],0,"Applying refresh-only records the new value in state, and updating code keeps the next normal plan from reverting it.","Objective 6d"],
    ["tf73",0,6,"Why should state files never be committed to a Git repository?",["Git cannot store JSON files","They may contain secrets in plain text and cause conflicts","Terraform deletes them after each run","They are too large to commit"],1,"State holds resource attributes including sensitive values, and shared editing through Git has no locking.","Objective 6c"],
    ["tf74",0,6,"You run terraform workspace new staging with the local backend. Where is the staging state stored?",["terraform.tfstate","terraform.tfstate.d/staging/terraform.tfstate","staging.tfvars","The .terraform/modules directory"],1,"Non-default CLI workspaces with the local backend keep state under terraform.tfstate.d/<name>/.","Objective 6a"],

    /* Domain 7: Maintain infrastructure (7) */
    ["tf75",0,7,"A container was created by hand. Which approach lets you import it and draft configuration in one plan?",["An import block and terraform plan -generate-config-out=generated.tf","terraform state push with a JSON file","terraform apply -replace on the container","A data block followed by terraform apply"],0,"Import blocks are planned like other changes, and -generate-config-out writes starter configuration for the imported resource.","Objective 7a"],
    ["tf76",0,7,"Which command lists every resource address tracked in the current state?",["terraform state list","terraform output","terraform providers","terraform graph"],0,"state list prints addresses; state show ADDRESS shows one resource's attributes.","Objective 7b"],
    ["tf77",0,7,"You renamed resource \"local_file\" \"a\" to \"local_file\" \"config\". How do you avoid Terraform destroying and recreating it?",["Add moved { from = local_file.a  to = local_file.config }","Add depends_on = [local_file.a]","Run terraform fmt","Run terraform init -upgrade"],0,"A moved block tells Terraform the object's address changed, so the plan shows a move. terraform state mv does the same imperatively.","Objective 7b"],
    ["tf78",0,7,"Terraform should stop managing a DNS record but leave it in place. Which option does that?",["terraform destroy -target on the record","A removed block with lifecycle { destroy = false }","Deleting the resource block and applying","lifecycle { prevent_destroy = true }"],1,"A removed block with destroy = false drops it from state without deleting it. Deleting the block and applying would destroy it.","Objective 7b"],
    ["tf79",0,7,"A provider fails with an unclear error and you need detailed logs saved to a file. Which settings do this?",["TF_LOG=DEBUG and TF_LOG_PATH=./tf.log","TF_VAR_log=debug","terraform plan -verbose","TF_INPUT=0"],0,"TF_LOG sets the log level and TF_LOG_PATH writes to a file. There is no -verbose flag on plan.","Objective 7c"],
    ["tf80",0,7,"You only want detailed logs from the provider plugin, not from Terraform core. Which variable should you set?",["TF_LOG_PROVIDER","TF_LOG_CORE","TF_CLI_ARGS","TF_WORKSPACE"],0,"TF_LOG_PROVIDER sets the provider log level separately; TF_LOG_CORE does the same for core.","Objective 7c"],
    ["tf81",0,7,"Which command shows a human-readable view of the whole current state or a saved plan file?",["terraform show","terraform validate","terraform fmt","terraform login"],0,"terraform show reads state (or a plan file) and prints it; -json gives machine-readable output.","Objective 7b"],

    /* Domain 8: HCP Terraform (9) */
    ["tf82",0,8,"A developer wants to run plans from their laptop but have them execute and store state in HCP Terraform. Which workflow is this?",["VCS-driven","CLI-driven","API-driven","Agent-only"],1,"The CLI-driven workflow starts runs from local terraform commands and executes them remotely.","Objective 8a"],
    ["tf83",0,8,"Which block connects a configuration to an HCP Terraform organization and workspace?",["cloud { organization = \"acme\" workspaces { name = \"app\" } }","provider \"hcp\" { workspace = \"app\" }","backend \"local\" { organization = \"acme\" }","module \"cloud\" { source = \"app.terraform.io\" }"],0,"The cloud block in the terraform block sets the organization and workspaces, then terraform init connects to them.","Objective 8a"],
    ["tf84",0,8,"The same cloud credentials must be available to 30 workspaces. What is the best HCP Terraform feature for this?",["A variable set applied to them or their project","Copying variables into each workspace","A terraform.tfvars file in each repo","A shared module output"],0,"Variable sets share variables across many workspaces or projects from one place.","Objective 8d"],
    ["tf85",0,8,"How do HCP Terraform projects help a platform team?",["They replace workspaces and hold one shared state","They group workspaces so access and settings apply to the group","They store provider binaries for teams","They are needed to run terraform fmt"],1,"Projects organize workspaces and let you grant team access at project level. Each workspace still has its own state.","Objective 8c"],
    ["tf86",0,8,"A security team needs every run blocked if a storage bucket is public. Which HCP Terraform feature enforces this?",["Policy as code with Sentinel or OPA","Run triggers","Cost estimation","The private registry"],0,"Policies evaluate each plan and can block runs that break rules.","Objective 8b"],
    ["tf87",0,8,"The network workspace changes, and the app workspace that reads its outputs must then plan automatically. Which feature connects them?",["Run triggers","Variable sets","Health assessments","Team tokens"],0,"Run triggers queue a run in one workspace after a successful apply in a source workspace.","Objective 8d"],
    ["tf88",0,8,"Which HCP Terraform feature periodically checks whether real infrastructure still matches a workspace's state?",["Health assessments (drift detection)","terraform fmt -check","The private module registry","Speculative plans on pull requests"],0,"Health assessments run drift detection and continuous validation of checks on a schedule.","Objective 8b"],
    ["tf89",0,8,"What does a VCS-driven workspace do when someone opens a pull request against its branch?",["Applies the change right away","Runs a speculative plan and reports it on the pull request","Deletes the workspace state","Locks the repository"],1,"Speculative plans show the impact of a pull request without applying. Apply happens after merge (with approval if required).","Objective 8a"],
    ["tf90",0,8,"Which command stores an API token so the CLI can authenticate with HCP Terraform?",["terraform login","terraform init -token","terraform workspace select","terraform providers lock"],0,"terraform login opens a browser to create a token and saves it in the CLI credentials file.","Objective 8a"]
  ]
});
